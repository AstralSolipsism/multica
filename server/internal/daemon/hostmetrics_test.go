...
func TestPlanQuotaCacheLifecycle(t *testing.T) {
	...
	if extras.PlanLimits != observed {
		t.Fatalf("PlanLimits = %+v, want the cached snapshot", extras.PlanLimits)
	}

	// Unknown runtimes carry no quota.
	if got := d.heartbeatExtrasFor("runtime-other").PlanLimits; got != nil {
		t.Fatalf("unknown runtime PlanLimits = %+v, want nil", got)
	}

	// Empty inputs are ignored rather than cached.
	d.recordRuntimePlanQuota("", observed)
	d.recordRuntimePlanQuota("runtime-1", nil)
	if got := d.heartbeatExtrasFor("runtime-1").PlanLimits; got != observed {
		t.Fatalf("nil overwrite changed the cache: %+v", got)
	}

	d.planQuotaCache.Delete("runtime-1")
	if got := d.heartbeatExtrasFor("runtime-1").PlanLimits; got != nil {
		t.Fatalf("deleted runtime PlanLimits = %+v, want nil", got)
	}
}
