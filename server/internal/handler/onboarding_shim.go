...
const (
	onboardingAssistantName       = "Mizuki"
	onboardingIssueTitle          = "Start here: learn Labrastro with Mizuki"
...
const onboardingAssistantDescription = "Built-in workspace assistant. Answers Labrastro questions and runs CLI operations."
...
// Mizuki agent created by this shim. Pre-v3 desktop submits a
...
const onboardingAssistantInstructions = `You are Mizuki, the built-in AI assistant for this Labrastro workspace. Your role is to help any member use Labrastro better — answer questions, give advice, and execute workspace operations on their behalf.
...
const onboardingIssueDescription = `Welcome to Labrastro.

This is your guided first run. Mizuki is assigned to this issue and will help you try the core workflow:

1. Read Mizuki's first comment.
2. Reply with something you want to build, fix, write, or plan.
3. @mention Mizuki when you want it to continue.
...
//
// Creates or reuses one "Mizuki" agent on the supplied runtime,
...
func enNoRuntimeIssueDescription() string {
	return strings.Join([]string{
		...
		"   which codex",
		"   codex --version",
		"5. Restart the Labrastro daemon:",
		...
		"",
		"When the runtime is connected, you can create Mizuki for a guided first run.",
	}, "\n")
}
...
func zhNoRuntimeIssueDescription() string {
	return strings.Join([]string{
		...
		"",
		"运行时连上后，你就可以创建 Mizuki，开始一次有智能体参与的上手引导。",
	}, "\n")
}
