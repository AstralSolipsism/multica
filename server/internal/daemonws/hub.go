...
// WithDaemonID records the runtime's owning daemon id on the lease. The
// heartbeat path needs it to key machine-level state (host metrics samples)
// without re-reading agent_runtime every beat.
...
// passed (not just the runtime id) so optional observational fields
// (plan_limits, metrics) reach the handler.
