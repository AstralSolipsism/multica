...
Sign in on the web (https://multica.outlune.com) or with Multica Desktop. Email verification codes and Google are both supported.
...
- **With Web, or to add another computer**: click **Add a computer** in the top right, then run the two commands from the dialog in a terminal on the target computer:
  ...
  **Windows**: download the `multica-cli-<version>-windows-<arch>.zip`
  archive from `https://multica.outlune.com/downloads/cli/v<version>/` and
  extract `multica.exe` onto your `PATH`, then run `multica setup`.
