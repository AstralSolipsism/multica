...
| `MULTICA_ZENMUX_MANAGEMENT_API_KEY` | 空 | ZenMux Management API キー（コンソール発行、推論キーとは別）。サブスクリプションクォータコレクター用で、このマシンから外部に出ません。`MULTICA_ZENMUX_LINK` でランタイムを関連付けるまで何も報告されません |
| `MULTICA_ZENMUX_LINK` | 空（関連付けなし） | ZenMux アカウントとランタイムの明示的な手動関連付け: `hermes` はこの daemon の全 hermes ランタイム、`hermes@<workspace-id>` は指定ワークスペースの hermes ランタイム、`hermes:profile:<profile-id>` は特定のカスタムランタイムプロファイルのみ、`hermes:builtin[@<workspace-id>]` は組み込み hermes ランタイムのみ（同一ワークスペースで組み込みとカスタムプロファイルが共存する場合の区別用）。複数はカンマ区切り。daemon はランタイムが実際に使う LLM ゲートウェイを推測せず、未関連付けのランタイムは "未報告" のままです。daemon 起動時に適用されます。エントリの追加・変更・削除後に再起動すると反映され、削除された関連付けは次のハートビートでそのランタイムのクォータ表示が直ちにクリアされます（24 時間の鮮度期限切れを待ちません） |
| `MULTICA_KIMI_QUOTA_POLL_INTERVAL` | `2m` | Kimi プラクォータコレクターのポーリング間隔。マシン上の Kimi Code ローカル Server API をプローブし、`kimi web` が動いていない間は "未報告" のまま静かに待機します。Bearer トークンはローカルサーバーが同一ユーザーの所有であることを証明した後にのみ送信されます（Linux/macOS 対応。他のプラットフォームではコレクターは動作せず "未報告" のままです） |
| `MULTICA_ZENMUX_QUOTA_POLL_INTERVAL` | `2m` | ZenMux プラクォータコレクターのポーリング間隔。422 レート制限応答時は遅延を指数的に延長します |
| `MULTICA_ZENMUX_API_BASE_URL` | 公式エンドポイント | ZenMux subscription-detail エンドポイントの上書き。テストやステージングプロキシ向け |
