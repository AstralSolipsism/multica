...
| `MULTICA_ZENMUX_LINK` | 空（関連付けなし） | ZenMux アカウントとランタイムの明示的な手動関連付け: `hermes` はこの daemon の全 hermes ランタイム、`hermes@<workspace-id>` は指定ワークスペースの hermes ランタイムのみ。複数はカンマ区切り。daemon はランタイムが実際に使う LLM ゲートウェイを推測せず、未関連付けのランタイムは "未報告" のままです。daemon 起動時に適用されます。エントリの追加・変更・削除後に再起動すると反映され、削除された関連付けは次のハートビートでそのランタイムのクォータ表示が直ちにクリアされます（24 時間の鮮度期限切れを待ちません） |
| `MULTICA_KIMI_QUOTA_POLL_INTERVAL` | `2m` | Kimi プラクォータコレクターのポーリング間隔。マシン上の Kimi Code ローカル Server API をプローブし、`kimi web` が動いていない間は "未報告" のまま静かに待機します |
