...
| `MULTICA_ZENMUX_MANAGEMENT_API_KEY` | 空（コレクター無効） | ZenMux Management API キー（コンソール発行、推論キーとは別）。このマシンの hermes ランタイムを ZenMux アカウントに結び付け、サブスクリプションクォータコレクターを有効化します。キーはこのマシンから外部に出ません |
