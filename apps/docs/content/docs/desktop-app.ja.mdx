...
内部リリースソース `https://multica.outlune.com/downloads/desktop/` から、OS と CPU アーキテクチャに合ったインストーラーを選びます:
...
新規インストールでは自動更新はデフォルトで無効です。バックグラウンドでの確認を希望する場合は **設定 → アップデート** で有効にしてください。すでに有効にしているインストールでは、その選択はそのまま維持されます。有効にすると、アプリはバックグラウンドで新しいバージョンを確認してダウンロードします。ダウンロードが完了したら、すぐに再起動してインストールすることも、次回終了時にインストールさせることもできます。**設定 → アップデート** ではいつでも手動で新しいバージョンを確認できます。
...
出力されたハッシュを、内部リリースソースが CLI アーカイブと一緒に公開している `checksums.txt`（`https://multica.outlune.com/downloads/cli/v<version>/checksums.txt`）の該当行と照合してください。
...
<Callout type="warning">
これらの除外を追加してよいのは、内部リリースソース（`https://multica.outlune.com/downloads/`）からインストールし、チェックサムが一致している場合だけです。除外を設定すると、そのフォルダー内のすべてのファイルでリアルタイム保護が無効になります。
</Callout>
