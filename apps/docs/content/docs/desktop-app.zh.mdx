...
从内部发布源 `https://multica.outlune.com/downloads/desktop/` 选择与系统和处理器架构对应的安装包：
...
全新安装默认关闭自动更新。如需后台检查新版本，可在 **设置 → 更新** 中开启；已经开启过的安装会保留原有选择。开启后，应用会在后台检查并下载新版本，下载完成后可以立即重启安装，也可以等下次退出时安装。你也可以随时在 **设置 → 更新** 中手动检查新版本。
...
把输出的哈希和内部发布源随 CLI 压缩包一起发布的 `checksums.txt`（`https://multica.outlune.com/downloads/cli/v<version>/checksums.txt`）对应那一行比对。
...
<Callout type="warning">
只有在你是从内部发布源（`https://multica.outlune.com/downloads/`）安装、且校验和对得上的前提下，才加这些排除项。排除项会关闭该文件夹内所有文件的实时保护。
</Callout>
