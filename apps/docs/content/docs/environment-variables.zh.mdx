...
| `MULTICA_ZENMUX_LINK` | 空（不关联任何运行时） | ZenMux 账户与运行时的显式人工关联：`hermes` 关联本 daemon 上的所有 hermes 运行时，`hermes@<workspace-id>` 只关联指定工作区的 hermes 运行时，多个条目用逗号分隔。daemon 不会推断运行时真实使用的 LLM 网关，未关联的运行时保持 "未上报"。配置在 daemon 启动时生效：新增、修改或删除条目后重启 daemon；被删除的关联会在下一次心跳时主动清除该运行时的配额展示，而不必等待 24 小时过期窗口 |
| `MULTICA_KIMI_QUOTA_POLL_INTERVAL` | `2m` | Kimi 套餐配额采集器的轮询间隔；采集器探测本机 Kimi Code 本地 Server API，`kimi web` 未运行时静默保持 "未上报" |
