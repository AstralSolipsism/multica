...
| `MULTICA_ZENMUX_LINK` | 空（不关联任何运行时） | ZenMux 账户与运行时的显式人工关联：`hermes` 关联本守护进程上的所有 hermes 运行时，`hermes@<workspace-id>` 只关联指定工作区的 hermes 运行时，`hermes:profile:<profile-id>` 只关联某个自定义运行时 profile（用于内置 hermes 与自定义 hermes profile 同工作区共存时精确区分），多个条目用逗号分隔。守护进程不会推断运行时真实使用的 LLM 网关，未关联的运行时保持 "未上报"。配置在守护进程启动时生效：新增、修改或删除条目后重启守护进程；被删除的关联会在下一次心跳时主动清除该运行时的配额展示，而不必等待 24 小时过期窗口 |
