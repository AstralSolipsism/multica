...
現在は Feishu/Lark、Slack、DingTalk、WeCom、Telegram に対応しています。Chat と実行の仕組みは共通です。Feishu/Lark は明示的な会話許可を使い、ほかの連携はメンバーのアカウント連携を使います。
...
| タスクの作成 | エージェントが確認して通常のツールで処理 | `/issue` スラッシュコマンド。エージェントが説明を整理してから作成 | `/issue` メッセージコマンド。入力内容からそのまま作成 | `/issue` メッセージコマンド。入力内容からそのまま作成 | `/issue` メッセージコマンド。入力内容からそのまま作成 |
...
3. Feishu/Lark は会話の許可と許可者の呼び出し権限を確認し、ほかの連携は送信者のアカウント連携とメンバー資格を確認します。
...
Feishu/Lark の `/issue` はエージェントへの会話テキストです。ほかの連携では、`/issue` はコマンドであり、チャットのターンではありません。Multica は元のプラットフォームに結果を返しますが、コマンド自体は Multica Chat に追加しません。Slack のネイティブスラッシュコマンドは、独立した非同期の実行作成フローで処理されます。
...
- Feishu/Lark はインストールと許可ごとに、グループ内のトピック、グループ、DM を分離します。
...
Feishu/Lark の参加者にプラットフォームのアカウントは不要です。[会話の許可](/lark-bot-integration)を設定してください。以下はほかの連携のルールです。個人通知の宛先連携は別に維持されます。
