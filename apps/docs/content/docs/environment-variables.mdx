...
| `MULTICA_WORKSPACES_ROOT` | `~/multica_workspaces` | Root directory for run working directories || `MULTICA_AGENT_TEMP_BASE` | `/tmp` (Linux/macOS) | Linux/macOS only. Parent directory for private per-run temp dirs; must be an existing, writable absolute directory, and an invalid value fails run startup instead of falling back to `/tmp`. Pick a short path — child tools may bind AF_UNIX sockets under it, and `sun_path` is limited to 108 bytes on Linux and 104 on macOS || `MULTICA_KEEP_ENV_AFTER_TASK` | `false` | Keep run directories for debugging || `MULTICA_ZENMUX_MANAGEMENT_API_KEY` | empty | ZenMux Management API key (console-issued, distinct from the inference key) for the subscription quota collector; the key never leaves this machine. Nothing is reported until `MULTICA_ZENMUX_LINK` associates runtimes with the account || `MULTICA_ZENMUX_LINK` | empty (no runtime linked) | Explicit manual association between the ZenMux account and runtimes: `hermes` links every hermes runtime on this daemon, `hermes@<workspace-id>` links one workspace's, `hermes:profile:<profile-id>` links a single custom runtime profile, and `hermes:builtin[@<workspace-id>]` links only built-in hermes runtimes (needed to tell built-in and custom profile instances apart when they coexist in one workspace); comma-separated for several. The daemon never infers a runtime's real LLM gateway, and unlinked runtimes stay "not reported". Applied at daemon start: add, edit, or remove entries and restart the daemon; a removed entry actively clears that runtime's quota display on the next heartbeat instead of waiting out the 24h staleness window || `MULTICA_KIMI_QUOTA_POLL_INTERVAL` | `2m` | Poll interval of the Kimi plan-quota collector, which probes the Kimi Code local Server API on this machine and silently stays "not reported" while `kimi web` is not running. The bearer token is sent only after the local server proves same-user ownership (Linux/macOS; on other platforms the collector stays off and runtimes remain "not reported") || `MULTICA_ZENMUX_QUOTA_POLL_INTERVAL` | `2m` | Poll interval of the ZenMux plan-quota collector; 422 rate-limit answers stretch the delay exponentially || `MULTICA_ZENMUX_API_BASE_URL` | official endpoint | Override of the ZenMux subscription-detail endpoint; for tests and staging proxies |
