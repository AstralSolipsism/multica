...
| `MULTICA_ZENMUX_MANAGEMENT_API_KEY` | empty (collector disabled) | ZenMux Management API key (console-issued, distinct from the inference key) that ties this machine's hermes runtimes to a ZenMux account and enables the subscription quota collector; the key never leaves this machine |
