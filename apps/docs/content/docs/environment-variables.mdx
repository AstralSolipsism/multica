...
| `MULTICA_ZENMUX_LINK` | empty (no runtime linked) | Explicit manual association between the ZenMux account and runtimes: `hermes` links every hermes runtime on this daemon, `hermes@<workspace-id>` links one workspace's, comma-separated for several. The daemon never infers a runtime's real LLM gateway, and unlinked runtimes stay "not reported". Applied at daemon start: add, edit, or remove entries and restart the daemon; a removed entry actively clears that runtime's quota display on the next heartbeat instead of waiting out the 24h staleness window |
| `MULTICA_KIMI_QUOTA_POLL_INTERVAL` | `2m` | Poll interval of the Kimi plan-quota collector, which probes the Kimi Code local Server API on this machine and silently stays "not reported" while `kimi web` is not running |
