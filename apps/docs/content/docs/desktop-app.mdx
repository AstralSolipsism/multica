...
Pick the installer that matches your OS and CPU architecture from the internal release source at `https://multica.outlune.com/downloads/desktop/`:
...
Auto-update is off by default on a fresh install. Turn it on under **Settings → Updates** if you want background checks — an install that already enabled it keeps that choice. When enabled, the app checks for and downloads new versions in the background; once a download finishes you can restart to install immediately or let it install on the next quit. You can always check for new versions manually under **Settings → Updates**.
...
Compare the hash with the matching line in the `checksums.txt` published alongside the CLI archives on the internal release source (`https://multica.outlune.com/downloads/cli/v<version>/checksums.txt`).
...
<Callout type="warning">
Only add these exclusions if you installed Multica from the internal release source (`https://multica.outlune.com/downloads/`) and the checksum matches. An exclusion disables real-time protection for everything in that folder.
</Callout>
