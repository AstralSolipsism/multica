...
目前支持飞书/Lark、Slack、钉钉、企业微信和 Telegram。它们复用 Chat 和执行机制。飞书/Lark 使用显式会话授权，其他渠道保留成员绑定。
...
| 创建任务 | 与智能体澄清后，由它使用正常工具处理 | `/issue` slash 命令，智能体整理描述后创建 | `/issue` 消息命令，按输入直接创建 | `/issue` 消息命令，按输入直接创建 | `/issue` 消息命令，按输入直接创建 |
...
3. 飞书/Lark 检查会话授权和授权人的智能体调用权限；其他渠道检查发送者绑定及成员身份。
4. 消息进入一段[智能体对话](/chat)，并创建一次运行。
5. 智能体的回复回到原来的私聊或讨论串。

没有 @ Bot 的频道消息不会触发智能体，也不会被加入它的对话上下文。

飞书/Lark 的 `/issue` 是交给智能体的会话文本。其他渠道中，`/issue` 是命令，不是对话轮次：Multica 会在原平台返回处理结果，但不会把命令加入 Multica Chat。Slack 原生 slash 命令仍按独立的异步创建任务流程处理。
...
飞书/Lark 参与者无需平台账号，请先配置[会话授权](/lark-bot-integration)。下列账号绑定规则适用于其他渠道；个人通知地址绑定仍独立保留。
