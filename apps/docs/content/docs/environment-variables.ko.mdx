...
| `MULTICA_ZENMUX_LINK` | 비어 있음(연결된 런타임 없음) | ZenMux 계정과 런타임의 명시적 수동 연결: `hermes`는 이 daemon의 모든 hermes 런타임, `hermes@<workspace-id>`는 지정 워크스페이스의 hermes 런타임, `hermes:profile:<profile-id>`는 특정 커스텀 런타임 프로필만 연결합니다(내장 hermes와 커스텀 hermes 프로필이 한 워크스페이스에 공존할 때 구분용). 여러 개는 쉼표로 구분합니다. daemon은 런타임이 실제로 사용하는 LLM 게이트웨이를 추론하지 않으며 연결되지 않은 런타임은 "미보고" 상태를 유지합니다. daemon 시작 시 적용됩니다. 항목을 추가·수정·삭제한 후 daemon을 재시작하면 반영되며, 삭제된 연결은 다음 하트비트에서 해당 런타임의 쿼터 표시를 즉시 지웁니다(24시간 신선도 만료를 기다리지 않습니다) |
