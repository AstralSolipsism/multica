...
export function proxy(req: NextRequest) {
  ...
  if (LEGACY_ROUTE_SEGMENTS.has(firstSegment)) {
    ...
    // `/` alone even with a session on the upstream marketing hosts, so
    // bouncing there dead-ends instead of reaching the app. /login already resolves an authenticated visitor
  ...
  // The upstream public hosts keep serving a marketing site at `/`. Visiting
  // such a host root must remain a public-site navigation even when a local
