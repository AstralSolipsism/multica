...
- `release.yml` builds the complete candidate only by explicit dispatch from fork `main`, after the workflow is enabled and `LABRASTRO_CANDIDATE_ENABLED=true`. Configure the `fork-candidate` environment approval/main policy first. Only the final job can write a fork draft; GoReleaser publishing remains disabled. No Homebrew/tap or GHCR publishing. Images are built and checked on disposable native runners; operations rebuild from that source on their own hosts.
- Candidate artifacts bind one Actions run ID and attempt. Rerun **all jobs**, never mix prior successful jobs into a new attempt. Existing Releases (drafts included) are refused. Inspect a failed upload and obtain explicit authorization before removing its incomplete draft; retain the tag. `complete` in `release-validation.json` means the mandatory build/byte/feed matrix passed; native installation, signing and production readiness remain separate. See `.github/RELEASING.md` for dispatch, recovery and acceptance.
